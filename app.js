$(document).ready(function() {
    ...
    function displaySearchHistory() {
        ...
        searchedCities.forEach(function (city) {
            ...
            let buttonCityEl = $('<button></button>').addClass('btn btn-light').text(city);
        ...
        //load the searchhistory to storage
        // function loadSearchHistory() {
        //     //get saved searchedcities from local storage
        //     var savedHistory = JSON.parse(localStorage.getItem("searchedCities"));
        ...
        function saveSearchResults() {
            //worked on with tutor
            const prevSearchIndex = searchedCities.indexOf(city);
            const notSearched = prevSearchIndex === -1;
            if(notSearched){
            ...
            else {//worked on with tutor
                const removed = searchedCities.splice(prevSearchIndex, 1);
                searchedCities.unshift(removed.pop());
            }
        }

        //make a button that will show previous clicked city
        ...
        $(document).on('click', '.city',function(evt){
            console.log('NOw I woork always')
        })
